...
function FormularioWizard() {
  ...
  function validarPaso() {
    ...
    if (pasoActual === 2) {
      ...
      var fechaEvento = new Date(formData.fecha + 'T12:00:00');
      var ahora = new Date();
      ...
      var violaciones = [];
      formData.servicios.forEach(function (s) {
        var minDias = s.categoriaAnticipacion || 0;
        if (minDias > 0 && diasDisponibles < minDias) {
          // Evitar duplicar la misma categoría
          var yaIncluida = violaciones.find(function (v) { return v.categoria === s.categoria; });
          if (!yaIncluida) {
            violaciones.push({
      ...
      if (violaciones.length > 0) {
        var mensajes = violaciones.map(function (v) {
          return v.categoria + ' requiere ' + v.diasRequeridos + ' días de anticipación';
        });
        showToast('Anticipación insuficiente: ' + mensajes.join('. '), 'error', 6000);
        return false;
      }
