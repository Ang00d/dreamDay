 ...
 *   🟡 amarillo: algunos ocupados/bloqueados
...
router.get('/mes-carrito', async function (req, res, next) {
  try {
    ...
    var totalCarrito = serviciosIds.length;
    ...
    // Buscar ocupaciones SOLO de los servicios del carrito
    var ocupaciones = await Disponibilidad.find({
      fecha: { $gte: fechaInicio, $lte: fechaFin },
      servicioId: { $in: serviciosIds },
      estado: { $ne: 'disponible' }
    }).select('fecha servicioId estado -_id');

    // Contar servicios ocupados por día (sin duplicar el mismo servicio)
    var ocupadosPorDia = {};
    ocupaciones.forEach(function (o) {
      if (!ocupadosPorDia[o.fecha]) {
        ocupadosPorDia[o.fecha] = new Set();
      }
      ocupadosPorDia[o.fecha].add(o.servicioId.toString());
    });

    // Generar calendario con colores basados en servicios del carrito
    var calendario = [];
    for (var dia = 1; dia <= diasEnMes; dia++) {
      var fechaDia = anio + '-' + mesStr + '-' + String(dia).padStart(2, '0');
      var serviciosOcupados = ocupadosPorDia[fechaDia] ? ocupadosPorDia[fechaDia].size : 0;

      var color;
      if (serviciosOcupados === 0) {
        color = 'verde';
      } else if (serviciosOcupados < totalCarrito) {
        color = 'amarillo';
      } else {
        color = 'rojo';
      }
